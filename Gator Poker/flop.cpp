...
Flop::Flop(vector<Person*>& player, vector<bool>& fold, int b, int p) {
	...
	while(true) {
		...
		if(fold[playerNum] == false) {
			...
			if(playerNum == 0) {
				...
				player[playerNum]->showTable(3);
				
				cout << player[playerNum]->getName() << " you have $" << player[playerNum]->getMoney() << endl;
				cin.clear();
				cout << player[playerNum]->getName() << " Would you like to (1) see your cards (2) fold (3) check (4) bet: " << endl;
				cin >> decision;
			...
			if(decision == 1) {
					...
					cout << endl;
					cout << player[playerNum]->getName() << " would you like to (2) fold (3) check (4) bet: " << endl;
					cin >> decision;
